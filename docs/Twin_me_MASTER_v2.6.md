...
**Target Platform:** React Native (iOS / Android, Expo)
**문서 일자:** 2026.07.05
...
| **2026.07.05** | **v2.6 (본 문서)** | **엔진 파일 교차 감사 반영: 오라 색상 정책 B안 확정(고정 hex 폐기 → HSL 동적 산출 단일화), 6축 라벨 코드 변수명 기준 통일(§1.3), `colors.ts` 구현 스펙 신규(§1.7), 점토 4단계 전환 조건 명세 신규(§3), 엔진 누락 로직 추가(coolingBleed §5.6, 위기 메모리 §5.7, scoreCalculator §5.8), 부록G 인라인 통합, 부록A·C·R 이식 명세로 전환, §12.2 구 버그트래커 → 신규 개발 체크포인트로 교체, §14 이식 엔진 파일 명세 신규 추가.** |
